...
begin
  if not private.is_leogo_admin('orders.manage')
     and not private.is_leogo_admin('approvals.read') then
    raise exception 'Aftersales management permission required';
  end if;
...
revoke execute on function public.admin_list_marketplace_aftersales() from public,anon;
revoke execute on function public.admin_update_marketplace_aftersales(uuid,text,text) from public,anon;

grant execute on function public.admin_list_marketplace_aftersales() to authenticated;
grant execute on function public.admin_update_marketplace_aftersales(uuid,text,text) to authenticated;
